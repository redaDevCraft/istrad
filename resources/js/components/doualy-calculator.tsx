...
const inputCls =
  "w-full px-2.5 py-1.5 text-[12px] bg-[#f7f6f2] border border-[rgba(14,17,23,0.1)] rounded-[6px] " +
  "text-[#0e1117] outline-none transition-all focus:border-[#0d6e56] focus:bg-white placeholder:text-[#bbb]";

function Field({ label, sub, children }: { label: string; sub?: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="block text-[10px] font-semibold uppercase tracking-widest text-[#9aa] mb-1">
...
function PillGroup<T extends string | number>({
  options, value, onChange, sm,
}: { options: { v: T; label: string }[]; value: T; onChange: (v: T) => void; sm?: boolean }) {
  return (
    <div className="flex gap-1 flex-wrap">
      {options.map(({ v, label }) => (
        <button key={String(v)} type="button" onClick={() => onChange(v)}
          className={`py-1 px-2.5 text-[11px] font-semibold rounded-[5px] border transition-all ${
            value === v
...
function Toggle({ checked, onChange, label }: { checked: boolean; onChange: (v: boolean) => void; label: string }) {
  return (
    <label className={`flex items-center gap-1.5 text-[11px] cursor-pointer px-2.5 py-1.5 rounded-[6px] border transition-all select-none ${
      checked
        ? "border-[#0d6e56] bg-[#e0f4ed] text-[#0d6e56] font-semibold"
        : "border-[rgba(14,17,23,0.1)] bg-[#f7f6f2] text-[#6b7080] hover:border-[#0d6e56]"
    }`}>
      <input type="checkbox" className="w-3 h-3 accent-[#0d6e56]" checked={checked} onChange={e => onChange(e.target.checked)} />
      {label}
    </label>
  );
}
...
function AnimatedValue({ value, bold, large }: { value: string; bold?: boolean; large?: boolean }) {
  ...
  return (
    <span className={`font-['DM_Mono',monospace] transition-opacity duration-100 tabular-nums ${
      flash ? "opacity-100" : "opacity-100"
    } ${large ? "text-[22px] font-bold tracking-tight" : bold ? "text-[13px] font-bold" : "text-[12px]"}`}>
      {display}
    </span>
  );
}
...
export default function DoualyCalculator() {
  ...
  useEffect(() => {
    if (form.currency !== "DZD") setForm(prev => ({ ...prev, exchangeRate: FX_RATES[form.currency] }));
  }, [form.currency]);
  ...
  return (
    ...
    <div className="flex h-full min-h-0 gap-0 overflow-hidden rounded-[16px] border border-[rgba(14,17,23,0.09)] shadow-sm bg-white">

      {/* ── LEFT: INPUTS (scrollable) ─────────────────────────────────── */}
      <div className="w-[55%] overflow-y-auto border-r border-[#eeece8] px-4 py-3 custom-scroll">
        ...
        <div className="grid grid-cols-2 gap-x-3 gap-y-2.5">
          ...
          <div className="col-span-2 grid grid-cols-2 gap-1.5 pb-3">
      ...
      <div className="w-[45%] flex flex-col bg-[#fafaf8] overflow-hidden">

        {/* KPI strip */}
        <div className="grid grid-cols-2 gap-0 border-b border-[#eeece8]">
          <div className="p-4 border-r border-[#eeece8] bg-[#7a818d]">
            <p className="text-[9px] font-bold uppercase tracking-widest text-white mb-1">Coût total rendu</p>
            <AnimatedValue value={`${fmt(results.totalLandedCost)}`} large  />
            <span className="text-[10px] text-white ml-1">DZD</span>
          </div>
          <div className="p-4 bg-[#64ac9a]">
            <p className="text-[9px] font-bold uppercase tracking-widest text-white mb-1">Coût / unité</p>
        ...
        <div className="grid grid-cols-2 gap-0 border-b border-[#eeece8]">
          <div className="p-3 border-r border-[#eeece8]">
            <p className="text-[9px] font-bold uppercase tracking-widest text-[#9aa] mb-0.5">Total taxes</p>
            <AnimatedValue value={`${fmt(results.totalTaxes)} DZD`} bold />
          </div>
          <div className="p-3">
            <p className="text-[9px] font-bold uppercase tracking-widest text-[#9aa] mb-0.5">Taux effectif</p>
        ...
        <div className="flex-1 overflow-y-auto custom-scroll px-3 py-2">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[9px] font-bold uppercase tracking-widest text-[#9aa]">Décomposition</span>
            <span className="flex items-center gap-1 text-[9px] text-[#0d6e56] font-semibold">
              <span className="w-1 h-1 rounded-full bg-[#0d6e56] animate-pulse inline-block" />
              Temps réel
            </span>
          </div>
          ...
          <div className="flex flex-col gap-0.5">
            {taxRows.map(({ label, value, highlight }) => (
              <div key={label} className={`flex justify-between items-center px-2.5 py-1.5 rounded-[5px] text-[11px] ${
            ...
            {/* Exchange rate line */}
            <div className="flex justify-between items-center px-2.5 py-1.5 rounded-[5px] bg-[#f0efeb] text-[11px]">
          ...
          {alerts.length > 0 && (
            <div className="flex flex-col gap-1 mt-3">
              {alerts.map((a, i) => (
                <div key={i} className={`px-2.5 py-1.5 rounded-[5px] text-[10px] flex items-start gap-1.5 leading-relaxed border ${
          ...
          {/* Formulas footer */}
          <div className="mt-3 pt-2 border-t border-[#e8e6e0]">
            <p className="text-[8.5px] text-[#bbb] leading-loose font-['DM_Mono',monospace]">
      ...
      {/* Scrollbar style (scoped via global injection) */}
      <style>{`
